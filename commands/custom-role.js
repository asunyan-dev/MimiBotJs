      ...
      sub
        ...
        .addStringOption((option) =>
          option
            .setName("color")
            .setDescription(
              "New color for the role, hex code must start with #"
            )
            .setRequired(true)
        )
    )
  ...
  async execute(interaction) {
    ...
    if (sub === "make") {
      if (status) {
        errorEmbed.setDescription(
          "You already have a custom role! Please use `/custom-role edit` to edit it!"
        );
        return interaction.reply({
          embeds: [errorEmbed],
          flags: MessageFlags.Ephemeral,
      ...
      if (!color.startsWith("#")) {
        errorEmbed.setDescription(
          "Invalid color format! It must start with `#`.\nExample: `#e410d3`."
        );
        return interaction.reply({
          embeds: [errorEmbed],
          flags: MessageFlags.Ephemeral,
      ...
      try {
        ...
        console.error(error);
        errorEmbed.setDescription(
          "Failed to make role. Please try again later."
        );
        return interaction.reply({
          embeds: [errorEmbed],
          flags: MessageFlags.Ephemeral,
    ...
    if (sub === "edit") {
      ...
      if (!role) {
        ...
        errorEmbed.setDescription(
          "Your custom role couldn't be found in the server. Please make another one using `/custom-role make`."
        );
        return interaction.reply({
          embeds: [errorEmbed],
          flags: MessageFlags.Ephemeral,
      ...
      try {
        ...
        console.error(error);
        errorEmbed.setDescription(
          "Failed to edit role, please try again later."
        );
        return interaction.reply({
          embeds: [errorEmbed],
          flags: MessageFlags.Ephemeral,
    ...
    if (sub === "remove") {
      ...
      try {
        ...
        console.error(error);
        errorEmbed.setDescription(
          "Failed to remove role, please try again later."
        );
        return interaction.reply({
          embeds: [errorEmbed],
          flags: MessageFlags.Ephemeral,
