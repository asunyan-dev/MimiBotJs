...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (sub === "enable") {
      if (status.enabled) {
        errorEmbed.setDescription(
          "Welcome messages are already enabled. Type `/welcoming edit` to edit them."
        );
    ...
    if (sub === "edit") {
      if (!status.enabled) {
        errorEmbed.setDescription(
          "Welcome messages are disabled. Type `/welcoming enable` to enable them."
        );
        return interaction.reply({
          embeds: [errorEmbed],
          flags: MessageFlags.Ephemeral,
    ...
    if (sub === "get") {
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          {
            name: "Ping new members?",
            value: status.ping ? "✅" : "❌",
            inline: true,
          },
          {
            name: "Channel",
            value: status.channelId ? `<#${status.channelId}>` : "N/A",
          }
        )
        .setTimestamp();
