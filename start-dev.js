...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.customId === "bug_report") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setAuthor({
        ...
        iconURL: interaction.user.avatarURL({ size: 128 }),
      })
      .setColor(0xe410d3)
    ...
    try {
      await sendMessage(client, config.mimicord, config.reports_channel, {
        embeds: [embed],
      });
  ...
  if (interaction.customId === "request") {
    ...
    try {
      await sendMessage(client, config.mimicord, config.requests_channel, {
        embeds: [embed],
      });
      ...
      errorEmbed.setDescription(
        "There was an error sending your request. You might want to either report a bug by using `/report-a-bug` or by joining the [support server](https://mimicord.com/)."
      );
      return interaction.reply({
        embeds: [errorEmbed],
        flags: MessageFlags.Ephemeral,
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.customId === "modmail") {
    ...
    try {
      await sendMessage(client, interaction.guild.id, channelId, {
        embeds: [embed],
        components: [row],
      });
  ...
  if (interaction.customId.startsWith("modmail_reply_")) {
    ...
    try {
      ...
      console.log(error);
      errorEmbed.setDescription(
        "Failed to send reply. User probably has DMs closed."
      );
      return interaction.reply({
        embeds: [errorEmbed],
        flags: MessageFlags.Ephemeral,
  ...
  if (interaction.customId.startsWith("user_reply_")) {
    ...
    try {
      await sendMessage(client, guildId, status.channelId, {
        embeds: [embed],
        components: [row],
      });
      successEmbed.setDescription("Reply sent.");
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.customId.startsWith("enable_welcome_")) {
    ...
    const channelValues = interaction.fields.getSelectedChannels(
      "channel",
      true
    );
    const message = interaction.fields.getTextInputValue("message");
...
client.on(Events.GuildMemberAdd, async (member) => {
  ...
  try {
    await sendMessage(client, member.guild.id, status.channelId, {
      content: content,
      embeds: [embed],
    });
  } catch (err) {
